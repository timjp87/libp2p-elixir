defmodule Libp2p.SwarmRuntimeTest do
  ...
  test "swarm can listen and accept a dial" do
    ...
    {:ok, swarm_a} = Swarm.start_link(name: nil, identity: id_a, peer_store: ps_a, connection_supervisor: cs_a)

    {:ok, ps_b} = PeerStore.start_link(name: nil)
    {:ok, cs_b} = DynamicSupervisor.start_link(strategy: :one_for_one)
    {:ok, swarm_b} = Swarm.start_link(name: nil, identity: id_b, peer_store: ps_b, connection_supervisor: cs_b)
